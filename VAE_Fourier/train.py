...
# want to test is the fourier domain data is robust enough or not.
import numpy as np
import os.path
...
with tf.Session() as sess:
    ...
    for step in range(1, int(n_steps)):
        ...
        data = batch[0].reshape((batch_size, 28, 28))
        fdata = np.absolute(np.fft.fft2(data))/100
        feed_dict = {x: fdata.reshape((batch_size, 28*28))}  ## add fourier transform here
